...
function Join(props) {
  let membersTotal = 0;
    ...
    const [examineStatus, setExamineStatus] = useState('加入家庭,与宝宝共同成长');
    ...
    function getDeviceInfo() {
        api.device_v1_info_sn_sn_get({
          ...
          path:{
            ...
            if(e.code===200)
            setDeviceInfo(Object.assign({},deviceInfo,{name:e.data.name || '小智伴'}))
        })
      };
      function getMemberData() {
        api.api_v3_device_info_members_deviceId_post({
             headers: {
                 // appId: this.appId,
                 deviceId:deviceId
             },
            ...
            //  console.info(e,"设备成员信息")
             ...
             e.code===200 && setMemberData(e.data)
         })
     };
     function getDeviceMembersTotal() {
        api.api_v3_device_info_members_total_deviceId_get({
          headers: {
            deviceId: deviceId
          },
          path:{
     ...
     function checkBindStatus() {
        api.api_v3_device_info_search_exists_post({
            headers: {
              appId: appId,
              deviceId: deviceId
            },
            data: {
             ...
             setBind(e.code ? e.data : e);  
             let flag = e.code ? e.data : e;
                if (!flag) {
                    if(membersTotal >= 10){
                      // 现有家庭成员已满10个（包含正在申请加入的用户），toast：当前家庭成员已满
                      setExamineStatus('当前家庭成员已满，期待您对宝宝的关心');
                      setBind(true);
                    }else{
                      setExamineStatus('加入家庭,与宝宝共同成长');
                    }
                    return;
                  }
                  getExamineStatus();
      ...
      async function getExamineStatus() {
        ...
        let flag = false;
        for (let i = 0; i <= 3; i++) {
          await result(i).then(e => {
            if (e.code&&e.data) {
              flag = true;
              // console.info(e, "根据条件查询绑定设备信息")
              // 审核状态0:待审核 1:通过 2:不通过
              switch (e.data.examine) {
                case 0:
                  setExamineStatus('你已申请加入家庭，请耐心等待管理员审核');
                  setBind(true);
                  break
                case 1:
                  setExamineStatus('你已经加入该家庭');
                  setBind(true);
                  break
                case 2:
                  setExamineStatus('审核不通过，请重新申请');
                  setBind(false);
                  break
                default:
                  setExamineStatus('加入家庭,与宝宝共同成长');
    ...
    return (
        <Wrapper className='family-join'>
            ...
            <Content>
                {(!!memberData.total) && memberData.content.map((item) => {
            ...
            <Footer>
            <Button className={isBind?null:'active'} onClick={joinHandler}>{examineStatus}</Button>
            </Footer>
           { isDialogShow && <CusDialog
            isShow={isDialogShow}
            title='你已经加入该家庭'
            content='是否打开智伴优学APP？'
            cancel={
